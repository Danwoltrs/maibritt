...
export function DesignBar({ title, grid, onGrid, onBack, onReset, onDone, canReset }: {
  ...
  return (
    <div
      className="absolute left-0 right-0 top-0 z-[65] flex items-center justify-between gap-3 overflow-x-auto px-4 py-1 backdrop-blur-sm md:px-6"
    ...
    >
      <div className="flex items-center gap-3">
        ...
        <span className="hidden whitespace-nowrap text-[18px] md:inline" style={{ color: 'var(--ink-2)' }}>{title}</span>
      </div>
